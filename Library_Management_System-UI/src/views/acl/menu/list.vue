<template>
  <div class="app-container">
    ...
    />
    <!-- <el-tooltip content="展开列表" placement="top">
      <el-switch
        v-model="isExpand_"
        active-color="#13ce66"
    ...
    >
      ...
      <el-table-column prop="permissionValue" align="center" label="权限值">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    //  树形表格过滤
    handleTreeData(treeData, filterText) {
      ...
      for (let i = 0; i < treeData.length; i += 1) {
        ...
        for (let pro in treeData[i]) {
          if (typeof treeData[i][pro] == "string") {
            match |= treeData[i][pro].includes(filterText);
            if (match) break;
          }
        }
        if (
          this.handleTreeData(treeData[i].children, filterText).length > 0 ||
          match
        ) {
          if (treeData[i].type == 0 && match) {
            ...
            array.push({
              ...treeData[i],
              children: this.handleTreeData(treeData[i].children, filterText),
            });
