<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="network" label="官网" align="center" />
      <el-table-column prop="address" label="地址" align="center" />
      <el-table-column prop="description" label="描述" align="center" />

      <el-table-column label="操作" width="200" align="center">
        <template slot-scope="scope">
          <router-link :to="'/lms/library/update/' + scope.row.id">
            <el-button
              type="primary"
              size="mini"
              icon="el-icon-edit"
              v-if="hasPerm('library.update')"
            ></el-button>
          </router-link>
          <el-button
            type="danger"
...
export default {
  ...
  methods: {
    ...
    addUser() {
      this.$router.push({ path: "/lms/library/add" });
    },
    saveRows() {
      this.$router.push({ path: "/lms/library/import" });
    },
    // 加载分类列表数据
    fetchData(page = 1) {
      console.log("翻页。。。" + page);
    ...
    removeDataById(id) {
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          // promise
          // 点击确定，远程调用ajax
          return libraryApi.removeById(id);
        })
        .then((response) => {
    ...
    removeRows() {
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          // 调用api
          return libraryApi.removeRows(idList);
        })
        .then((response) => {
          this.fetchData(this.page);
          if (response.success) {
